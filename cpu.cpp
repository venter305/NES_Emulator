...
int cpu::ADC(){
	...
	if(instrVal == -1) instrVal = nes->readMemory(instrAddr);
...
int cpu::DEC(){
	...
	p &= ~P_NEGATIVE;
	p |= (nes->readMemory(instrAddr) & P_NEGATIVE);
	//Set Zero flag
...
int cpu::INC(){
	...
	if (nes->readMemory(instrAddr) > 0xFF)
		nes->writeMemory(instrAddr, nes->readMemory(instrAddr) - 0x100);
